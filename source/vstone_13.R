...
output <- here("images", "vstone_13.png")
...
jas <- use_seed(34) %>%
  scene_sticks(n = 5, grain = 1000) %>%
  mutate(ind = 1:n(), x = x * 10, y = y * 10) %>%
  unfold_breeze(scale = .001, iterations = 200) %>%
  mutate(x = normalise(x, to = c(0, 1)),
