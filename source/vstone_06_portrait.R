...
output <- here("images", "vstone_06_portrait_A4.png")
...
shades <- 1000
scf <- 1
grains_wide <- round(525 * scf)
grains_high <- round(740 * scf)
...
palette2 <- scico::scico(
  palette = "lajolla", 
  n = shades,
  alpha = .6
)

#bg <- palette[shades*.65]
bg <- palette[shades*.9]
...
pic <- ggplot(
  ...
  # bunch of settings...
  scale_color_paletteer_c("scico::lajolla", limits = c(-50, 25)) + 
  scale_fill_identity() + 
  scale_size_identity() + 
  coord_equal() + 
