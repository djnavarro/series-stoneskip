...
output <- here("images", "vstone_20.png")
...
shades <- 1000
scf <- 1
grains_high <- round(600 * scf)
grains_wide <- round(600 * scf)

palette <- paletteer_c(
  palette = "gameofthrones::stark", 
  n = shades
)
...
pic <- ggplot(
  ...
  # bunch of settings...
  scale_color_paletteer_c("gameofthrones::stark", limits = c(-50, 25)) + 
  scale_fill_identity() + 
  scale_size_identity() + 
  coord_equal() + 
